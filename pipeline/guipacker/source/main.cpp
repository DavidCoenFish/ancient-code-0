...
{
	mMessage->AddMessage(std::string("GuiPacker::Run(") + in_sourceXml + ", " + in_destFile + ")");
	...
	{
		mMessage->AddError(std::string("PackGui threw error:"), true);
    }
...
int main(int in_argc, char ** in_argv)
{
	...
	if (3 != in_argc)
	{
		printf( "\nusage:\\GuiPacker.exe src.xml destFileName.xml\nchange Gui files into fudge packer xml source\n" ); 
	}
	else
	{
