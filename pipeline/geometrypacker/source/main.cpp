...
{
	mMessage->AddMessage(std::string("GeometryPacker::Run(") + in_sourceXml + ", " + in_destFile + ")");
	...
	{
		mMessage->AddError(std::string("PackGeometry threw error:"), true);
    }
...
int main(int in_argc, char ** in_argv)
{
	...
	if (3 != in_argc)
	{
		printf( "\nusage:\\GeometryPacker.exe src.xml destFileName.xml\nchange geometry files into fudge packer xml source\n" ); 
	}
	else
	{
