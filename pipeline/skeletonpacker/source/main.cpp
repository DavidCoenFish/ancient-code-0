...
{
	mMessage->AddMessage(std::string("SkeletonPacker::Run(") + in_sourceXml + ", " + in_destFile + ")");
	...
	{
		mMessage->AddError(std::string("SkeletonPacker threw error:"), true);
    }
...
int main(int in_argc, char ** in_argv)
{
	...
	if (3 != in_argc)
	{
		printf( "\nusage:\\SkeletonPacker.exe src.xml destFileName.xml\nchange skeleton xml file into fudge packer xml source\n" ); 
	}
	else
	{
