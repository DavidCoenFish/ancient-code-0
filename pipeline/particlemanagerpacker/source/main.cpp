...
{
	mMessage->AddMessage(std::string("ParticleManagerPacker::Run(") + in_sourceXml + ", " + in_destFile + ")");
	...
	{
		mMessage->AddError(std::string("PackMission threw error:"), true);
    }
...
int main(int in_argc, char ** in_argv)
{
	...
	if (3 != in_argc)
	{
		printf( "\nusage:\\MissionPacker.exe src.xml destFileName.xml\nchange mission files into fudge packer xml source\n" ); 
	}
	else
	{
