...
{
	mMessage->AddMessage(std::string("AnimationPacker::Run(") + in_sourceXml + ", " + in_destFile + ")");
	...
	{
		mMessage->AddError(std::string("PackAnimation threw error:"), true);
    }
...
int main(int in_argc, char ** in_argv)
{
	...
	if (3 != in_argc)
	{
		printf( "\nusage:\\AnimationPacker.exe src.xml destFileName.xml\nchange animation files into fudge packer xml source\n" ); 
	}
	else
	{
