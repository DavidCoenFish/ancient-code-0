	...
	)
{
	...
	pointerStateFactory.reset(new PlayWithGoldfishStateFactory(inout_dictionary));
...
//debug
	...
	{
		float sumError = 0.0F;
		float sumErrorCount = 0;
		for (float value = 0.0F; value < 4.0F; value += 0.1F)
		{
			const float resultCorrect = GMath::Sqrt(value);
			const float resultApprox = GMath::SqrtAprox(value);
			if (resultCorrect)
			{
				const float error = GMath::Abs(resultApprox - resultCorrect);
				sumError += error;
				sumErrorCount += 1.0F;
			}
		}
		if (sumErrorCount)
		{
			const float resultError = sumError / sumErrorCount;
			GASSERT(resultError < 0.03F, "accuracy check");
		}
	}
	...
	//ApproxSqrt
	{
		float sumError = 0.0F;
		float sumErrorCount = 0;
		for (float value = 0.1F; value < 4.0F; value += 0.1F)
		{
			const float resultCorrect = 1.0F / GMath::Sqrt(value);
			const float resultApprox = GMath::InvertSqrtAprox(value);
			if (resultCorrect)
			{
				const float error = GMath::Abs(resultApprox - resultCorrect);
				sumError += error;
